...
class Decoder:
    def __init__(self, debug=True, coder=EmptyEcc(), setting=Settings()):
        self.win_len = 2 * int(setting.bit_duration *
                               setting.rate / setting.chunk_size / 2)
    ...
    def listen(self):
        ...
        while not self.do_quit:
            ...
            self.audio = list(struct.unpack("%dh" %
                                            self.setting.chunk_size, audiostr))
            self.window()
            powerlist = np.array([self.goertzel(i)
                                  for i in self.setting.char_freq])
    ...
    def window(self):
        self.audio = [aud * win for aud,
                      win in zip(self.audio, self.setting.window)]
