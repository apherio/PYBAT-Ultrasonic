...
from pybat.codec.conf import Settings
...
class Encoder:
    ...
    def string2sound(self, somestring):
        binform = ''.join('-001' + self.coder.get_encoded_bytes_string(i)
                          for i in somestring) + '-010'  #
        if self.debug:
            print("binform:" + binform)
        # After converting it to binary number, divide it by 2 bits and convert each to decimal number
        multiple = [int(binform[i:i + 4], 2)
                    for i in range(len(binform)) if i % 4 == 0]  #
        if self.debug:
    ...
    def encode2wav(self, somestring, filename):
        ...
        wavfile.write(filename, self.setting.rate,
                      soundlist.astype(np.dtype('int16')))
    ...
    def getbit(self, freq):
        music = []
        t = np.arange(0, self.setting.bit_duration,
                      1. / self.setting.rate)  # time
        x = np.sin(2 * np.pi * freq * t)  # generated signals
        x = [int(val * 32000) for val in x]

        sigmoid = [1 / (1 + np.power(np.e, -t))
                   for t in np.arange(-6, 6, 0.02)]  # 0.01
